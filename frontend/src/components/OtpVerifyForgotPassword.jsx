...
const OtpVerifyForgotPassword = ()=>{
    ...
    const submitHandle = async (e)=>{
        e.preventDefault();
        try {
            const getOTP = await fetch(`http://localhost:5050/user/forgotPassword/OTP_verification/${email}`,{
            method : "POST",
            headers : {
            ...
            const data = await getOTP.json();
            if(getOTP.ok){
                navigate(`/user/renewPassword/${email}`);
                console.log('both password should be equal');
            }else{
